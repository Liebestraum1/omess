...
type IssueCreateChargerFilterProp = {
    onChangeCharger: (status: number | null) => void;
}

const IssueCreateChargerFilter = ({onChangeCharger} :IssueCreateChargerFilterProp) => {
    const {projectMembers} = useKanbanBoardStore(state => ({
    ...
    const handleChange = (event: SelectChangeEvent<string>) => {
        ...
        if (event.target.value == "") {
            onChangeCharger(null);  // Zustand 스토어 상태 업데이트
        } else {
            onChangeCharger(parseInt(event.target.value));  // Zustand 스토어 상태 업데이트
        }
    };
...
export default IssueCreateChargerFilter;