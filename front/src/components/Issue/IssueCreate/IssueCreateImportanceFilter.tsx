...
type IssueCreateImportanceFilterProp = {
    onChangeImportance: (status: number | null) => void;
}


const IssueCreateImportanceFilter = ({onChangeImportance} : IssueCreateImportanceFilterProp) => {

    const [selectedImpotance, setSelectedImpotanceLocal] = useState('');
    const handleChange = (event: SelectChangeEvent<string>) => {
        setSelectedImpotanceLocal(event.target.value);
        if (event.target.value == "") {
            onChangeImportance(null);
        } else {
            onChangeImportance(parseInt(event.target.value));
        }
    };
...
export default IssueCreateImportanceFilter;