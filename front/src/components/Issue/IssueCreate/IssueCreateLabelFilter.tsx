...
type IssueCreateChargerFilterProp = {
    onChangeLabel: (status: number | null) => void;
}

const IssueCreateLabelFilter = ({onChangeLabel} : IssueCreateChargerFilterProp) => {
    const {labels} = useKanbanBoardStore(state => ({
        labels: state.labels,
    }));
    ...
    const handleChange = (event: SelectChangeEvent<string>) => {
        ...
        if (event.target.value == "") {
            ...
            onChangeLabel(parseInt(event.target.value));
        }
    };
...
export default IssueCreateLabelFilter;